...
mydataset <- read.csv(here("Data/gen_data_final_distance_pop-level.csv") )
mydataset$Nucleotide_diversity <- mydataset$He_harm
mydataset$Latitude <- mydataset$lat
mydataset$dist_cent <- mydataset$min_dist
...
Genetic diversity (GD) data was extracted from Fonseca et al. (2023), describing patterns of intraspecific Genetic diversity. Genetic diversity was calculated from aggregated georeferenced mitochondrial DNA for animals (vertebrates, arachnids, and insects) and chloroplast DNA for plants using `phylogatR` (Pelletier et al., 2021; <https://phylogatr.osc.edu>), an open-source platform available through the Ohio Supercomputer Center (OSC). `phylogatR` retrieves genetic and geographic data by integrating information from the GBIF (<https://www.gbif.org/>), GenBank (<https://www.ncbi.nlm.nih.gov/genbank/>), and the BOLD (<http://www.boldsystems.org/index.php>). To minimize data loss and duplication when downloading data from GenBank, duplicate entries with the same accession number, but different GBIF occurrence numbers are checked for similarities in geographic coordinates. Sequences with high values of divergence were removed (likely resulting from inclusion of a sequence with an incorrect species label), a conservative approach that prevents inflation on the calculation of genetic diversity within species. Genetic diversity was calculated as the number of polymorphic sites in pairwise sequence comparison, using a modified version of the equation proposed by Nei and Li (1979) to allow for a comparison of sequences of different lengths (Miraldo et al. 2016). Finally, Genetic diversity between pairs of sequences were averaged to obtain a global estimation of Genetic diversity for each species, and associated centroid of georeferenced DNA sequences.
...
#  2 Aves           LE       93
#  3 Aves           TE       25
#  4 Actinopterygii O        87
#  5 Actinopterygii TE       13
#  6 Magnoliopsida  O        59
#  7 Magnoliopsida  LE       14
#  8 Magnoliopsida  TE       14
#  9 Liliopsida     O         9
# 10 Pinopsida      O        14
# 11 Pinopsida      LE        8
# 12 Mammalia       LE        6\
...
tmp <- mydatatogo %>%
  mutate(vel_abs=log1p(vel_abs),
         SHIFT_abs=log1p(SHIFT_abs)) %>%
  select(Class,GD,vel_abs,SHIFT_abs) %>%
...
ggplot(mydatatogo %>%
         select(Class,GD,vel_abs,SHIFT_abs) %>%
         gather(variable, value, -Class) %>%
         mutate(Class=factor(Class,levels = avg_gd$Class[order(avg_gd$avg)])), 
       aes(value))+
...
p1 <- ggplot(mydatatogo, aes(SHIFT_abs))+
  ...
  scale_x_log10()+
  theme_classic() +
  labs(y = "", x = bquote('Absolute velocity of range shift '(km.yr^1)))
...
p3 <- ggplot(mydatatogo, aes(vel_abs))+
  # geom_histogram(binwidth = .5,aes(fill=Class),alpha=.5) +
  geom_density(aes(fill=Class),alpha=.5) +
  # scale_x_log10()+
  theme_classic() +
...
ggplot(mydatatogo, aes(GD))+
  ...
  theme_classic() +
  labs(y = "Density", x = "Genetic diversity (log)")
...
pdf_file <- here::here("Figs/Map_pop-level_GD2.pdf")
pdf_page <- pdf_render_page(pdf_file, page = 1)
png_file <- here::here("Figs/Map_pop-level_GD2.png")
...
ex=pblapply(1:nB, function(i) {
  ...
  mydatatogo2=merge(mydatatogo2[,1:43],x1[,c(1,3)],by.x="spp",by.y="Var1")
...
write.table(r2,
            here::here("Output_pop-level",paste0("R2_",nom,".csv")),
            sep=";",dec=".",row=F)
write.table(coeff,
            here::here("Output_pop-level",paste0("coeff_",nom,".csv")),
            sep=";",dec=".",row=F)
write.table(randEff,here::here("Output_pop-level",paste0("randEff_",nom,".csv")),
            sep=";",dec=".",row=F)
...
ex=pblapply(1:nB, function(i) {
  ...
  x1=subset(x1,Freq>0)
  x1$weight_obs_spp=(1/x1$Freq)*(1/nrow(x1))
  mydatatogoCE=merge(mydatatogoCE[,1:43],x1[,c(1,3)],by.x="spp",by.y="Var1")
  
  mydatatogoLE=subset(mydatatogo2,Param=="LE")
  x1=data.frame(table(mydatatogoLE$spp))
  x1=subset(x1,Freq>0)
  x1$weight_obs_spp=(1/x1$Freq)*(1/nrow(x1))
  mydatatogoLE=merge(mydatatogoLE[,1:43],x1[,c(1,3)],by.x="spp",by.y="Var1")
  
  mydatatogoTE=subset(mydatatogo2,Param=="TE")
  x1=data.frame(table(mydatatogoTE$spp))
  x1=subset(x1,Freq>0)
  x1$weight_obs_spp=(1/x1$Freq)*(1/nrow(x1))
  mydatatogoTE=merge(mydatatogoTE[,1:43],x1[,c(1,3)],by.x="spp",by.y="Var1")
...
x1=subset(x1,Freq>0)
x1$weight_obs_spp=(1/x1$Freq)*(1/nrow(x1))
mydatatogoS=merge(mydatatogoS[,1:43],x1[,c(1,3)],by.x="spp",by.y="Var1")
...
resR2_ok_file <- here::here("Output_pop-level","summary_R2.csv")
res_ok_file <- here::here("Output_pop-level","summary_coeff.csv")
...
if(all(file.exists(resR2_ok_file,res_ok_file))){
  ...
  for(i in 1:length(mod)){
    print(a)
    r2=read.csv2(here::here("Output_pop-level",paste0("R2_",mod[i],".csv")),
                 sep=";",dec=".")
    #r2$nB=1:nrow(r2)
    ...
    if(nrow(r2a)<nB){
      ...
      c1=read.csv2(here::here("Output_pop-level",paste("coeff_",mod[i],".csv",sep="")),
                   sep=";",dec=".",h=T)
  ...
  write.table(resR2_ok,
              here::here("Output_pop-level","summary_R2.csv"),
              sep=";",dec=".",row=F) 
  ...
  write.table(res_ok,
              here::here("Output_pop-level","summary_coeff.csv"),
              sep=";",dec=".",row=F) 
...
```{r eval=FALSE}
res <- here::here("Output_pop-level","summary_GDeff_allW.csv")

if(!file.exists(res)){
  
  c1=read.csv2(here::here("Output_pop-level","coeff_allW.csv"),
               sep=";",dec=".",h=T)
  dsel=mydatatogo
  ...
  write.table(res,
              here::here("Output_pop-level","summary_GDeff_allW.csv"),
              sep=";",dec=".",row=F) 
...
```{r eval=FALSE}
res <- here::here("Output_pop-level","summary_GDeff_CEW.csv")

if(!file.exists(res)){
  
  c1=read.csv2(here::here("Output_pop-level","coeff_CEW.csv"),
               sep=";",dec=".",h=T)
  ...
  write.table(res,
              here::here("Output_pop-level","summary_GDeff_CEW.csv"),
              sep=";",dec=".",row=F) 
...
```{r eval=FALSE}
res <- here::here("Output_pop-level","summary_GDeff_LEW.csv")

if(!file.exists(res)){
  
  c1=read.csv2(here::here("Output_pop-level","coeff_LEW.csv"),
               sep=";",dec=".",h=T)
  ...
  write.table(res,
              here::here("Output_pop-level","summary_GDeff_LEW.csv"),
              sep=";",dec=".",row=F) 
...
```{r eval=FALSE}
res <- here::here("Output_pop-level","summary_GDeff_TEW.csv")

if(!file.exists(res)){
  
  c1=read.csv2(here::here("Output_pop-level","coeff_TEW.csv"),
               sep=";",dec=".",h=T)
  ...
  write.table(res,
              here::here("Output_pop-level","summary_GDeff_TEW.csv"),
              sep=";",dec=".",row=F) 
...
resTE <- here::here("Output_pop-level","summary_GDeff_allW2_TE.csv")
resCE <- here::here("Output_pop-level","summary_GDeff_allW2_CE.csv")
resLE <- here::here("Output_pop-level","summary_GDeff_allW2_LE.csv")

if(!all(file.exists(resTE,resCE,resLE))){
  
  c1=read.csv2(here::here("Output_pop-level","coeff_allW2.csv"),
               sep=";",dec=".",h=T)
  dsel=mydatatogo
  v1=seq(round(min(dsel$vel_abs),1),round(max(dsel$vel_abs),1),by=0.1)
  ...
  write.table(resTE,
              here::here("Output_pop-level","summary_GDeff_allW2_TE.csv"),
              sep=";",dec=".",row=F) 
  write.table(resCE,
              here::here("Output_pop-level","summary_GDeff_allW2_CE.csv"),
              sep=";",dec=".",row=F) 
  write.table(resLE,
              here::here("Output_pop-level","summary_GDeff_allW2_LE.csv"),
              sep=";",dec=".",row=F) 
...
```{r eval=FALSE}
res <- here::here("Output_pop-level","summary_VAeff_allW.csv")
...
if(!file.exists(res)){
  ...
  write.table(res,
              here::here("Output_pop-level","summary_VAeff_allW.csv"),
              sep=";",dec=".",row=F) 
...
```{r eval=FALSE}
res <- here::here("Output_pop-level","summary_VAeff_CEW.csv")
...
if(!file.exists(res)){
  ...
  write.table(res,
              here::here("Output_pop-level","summary_VAeff_CEW.csv"),
              sep=";",dec=".",row=F) 
...
```{r eval=FALSE}
res <- here::here("Output_pop-level","summary_VAeff_LEW.csv")
...
if(!file.exists(res)){
  ...
  write.table(res,
              here::here("Output_pop-level","summary_VAeff_LEW.csv"),
              sep=";",dec=".",row=F) 
...
```{r eval=FALSE}
res <- here::here("Output_pop-level","summary_VAeff_TEW.csv")

if(!file.exists(res)){
  
  c1=read.csv2(here::here("Output_pop-level","coeff_TEW.csv"),sep=";",dec=".",h=T)
  dsel=subset(mydatatogo,Param=="TE")
  v1=seq(round(min(dsel$GD),2),round(max(dsel$GD),2),by=0.01)
  ...
  write.table(res,
              here::here("Output_pop-level","summary_VAeff_TEW.csv"),
              sep=";",dec=".",row=F) 
...
```{r eval=FALSE}
resTE <- here::here("Output_pop-level","summary_VAeff_allW2_TE.csv")
resCE <- here::here("Output_pop-level","summary_VAeff_allW2_CE.csv")
resLE <- here::here("Output_pop-level","summary_VAeff_allW2_LE.csv")

if(!all(file.exists(resTE,resCE,resLE))){
  
  c1=read.csv2(here::here("Output_pop-level","coeff_allW2.csv"),sep=";",dec=".",h=T)
  dsel=mydatatogo
  ...
  write.table(resTE,
              here::here("Output_pop-level","summary_VAeff_allW2_TE.csv"),
              sep=";",dec=".",row=F) 
  write.table(resLE,
              here::here("Output_pop-level","summary_VAeff_allW2_LE.csv"),
              sep=";",dec=".",row=F) 
  write.table(resCE,
              here::here("Output_pop-level","summary_VAeff_allW2_CE.csv"),
              sep=";",dec=".",row=F) 
...
c1=read.csv2(here::here("Output_pop-level","summary_R2.csv"),
             sep=";",dec=".",h=T)
c1_m <- c1 %>%
...
```{r}
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
...
ggplot(allWtab %>%
         rowwise() %>%
         mutate(`Avg. coeff. + Std. error` = as.numeric(strsplit(`Avg. coeff. (SD)`, " ")[[1]][1]),
       ...
       aes(x = `Avg. coeff. + Std. error`, y = `Predictor variable`, color = sig)) +
...
```{r }
res=read.csv2(here::here("Output_pop-level","summary_GDeff_allW.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
...
res=read.csv2(here::here("Output_pop-level","summary_GDeff_TEW.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
c1a=subset(c1,model=="allW")
...
res=read.csv2(here::here("Output_pop-level","summary_GDeff_CEW.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
c1a=subset(c1,model=="allW")
...
res=read.csv2(here::here("Output_pop-level","summary_GDeff_LEW.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
c1a=subset(c1,model=="allW")
...
```{r}
## allW2_TE GDeff
res=read.csv2(here::here("Output_pop-level","summary_GDeff_allW2_TE.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
...
```{r}
res=read.csv2(here::here("Output_pop-level","summary_GDeff_allW2_CE.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
...
res=read.csv2(here::here("Output_pop-level","summary_GDeff_allW2_LE.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
...
res=lapply(c("LE","CE","TE"), function(x){
  tmp <- read.csv2(here::here("Output_pop-level",paste0("summary_GDeff_allW2_","TE",".csv")),
                   sep=";",dec=".",h=T)
  tmp$edge = x
  return(tmp)
})
...
res=read.csv2(here::here("Output_pop-level","summary_VAeff_allW.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
c1a=subset(c1,model=="allW")
...
res=read.csv2(here::here("Output_pop-level","summary_VAeff_TEW.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
c1a=subset(c1,model=="allW")
...
res=read.csv2(here::here("Output_pop-level","summary_VAeff_CEW.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
c1a=subset(c1,model=="allW")
...
res=read.csv2(here::here("Output_pop-level","summary_VAeff_LEW.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
c1a=subset(c1,model=="allW")
...
```{r}
## allW2_TE VAeff
res=read.csv2(here::here("Output_pop-level","summary_VAeff_allW2_TE.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
c1a=subset(c1,model=="allW2")
...
res=read.csv2(here::here("Output_pop-level","summary_VAeff_allW2_CE.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
...
res=read.csv2(here::here("Output_pop-level","summary_VAeff_allW2_LE.csv"),
              sep=";",dec=".",h=T) 
c1=read.csv2(here::here("Output_pop-level","summary_coeff.csv"),
             sep=";",dec=".",h=T)
