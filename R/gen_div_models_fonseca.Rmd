...
# pdf_file <- here::here("Figs/Map2.pdf")
png_file <- here::here("Figs/Map2.png")
# 
...
clusterExport(cl = my.cluster2, varlist = c("s1","nB","glmmTMB","mydatatogoS","gam_formulaLE","ranef"))
...
clusterExport(cl = my.cluster2, varlist = c("s1","nB","glmmTMB","mydatatogoS","gam_formulaTE","ranef"))
...
clusterExport(cl = my.cluster2, varlist = c("s1","nB","glmmTMB","mydatatogoS","gam_formulaCE","ranef"))
...
gg1 <- gg1 + theme(legend.position="none") + ggtitle("") + theme(plot.title = element_text(hjust = 0.5))
...
# reduce resolution for better plotting
# make a 10x10 matrix
toplot <- predX2[,c("GD","vel_abs","pred1")]

toplot$GD_breaks <- cut(toplot$GD,10)
toplot$vel_abs_breaks <- cut(toplot$vel_abs,10)

toplot2 <- toplot %>%
  group_by(GD_breaks,vel_abs_breaks) %>%
  summarise(pred1_avg = mean(pred1))

toplot2$GD_breaks_min <- sapply(toplot2$GD_breaks, function(x){
  tmp <- strsplit(as.character(x), ",")[[1]][1]
  as.numeric(gsub("(","",tmp,fixed = TRUE))
})
toplot2$GD_breaks_max <- sapply(toplot2$GD_breaks, function(x){
  tmp <- strsplit(as.character(x), ",")[[1]][2]
  as.numeric(gsub("]","",tmp,fixed = TRUE))
})
toplot2$GD_breaks_min[which(toplot2$GD_breaks_min==min(toplot2$GD_breaks_min))] <- min(mydatatogo$GD)
toplot2$GD_breaks_max[which(toplot2$GD_breaks_max==max(toplot2$GD_breaks_max))] <- max(mydatatogo$GD)
toplot2$vel_abs_breaks_min <- sapply(toplot2$vel_abs_breaks, function(x){
  tmp <- strsplit(as.character(x), ",")[[1]][1]
  as.numeric(gsub("(","",tmp,fixed = TRUE))
})
toplot2$vel_abs_breaks_max <- sapply(toplot2$vel_abs_breaks, function(x){
  tmp <- strsplit(as.character(x), ",")[[1]][2]
  as.numeric(gsub("]","",tmp,fixed = TRUE))
})
toplot2$vel_abs_breaks_min[which(toplot2$vel_abs_breaks_min==min(toplot2$vel_abs_breaks_min))] <- min(mydatatogo$vel_abs)
toplot2$vel_abs_breaks_max[which(toplot2$vel_abs_breaks_max==max(toplot2$vel_abs_breaks_max))] <- max(mydatatogo$vel_abs)

# add data density
toplot2$N <- NA
for(i in 1:nrow(toplot2)){
  tmp <- mydatatogo %>%
    filter(GD >= toplot2$GD_breaks_min[i] & GD <= toplot2$GD_breaks_max[i] & 
             vel_abs >= toplot2$vel_abs_breaks_min[i] & vel_abs <= toplot2$vel_abs_breaks_max[i])
  toplot2$N[i] <- nrow(tmp)
}

surfplot <- xtabs(pred1_avg ~ vel_abs_breaks_min + GD_breaks_min, 
                  data = toplot2 %>% filter(N>0))
surfplot[surfplot==0] <- NA

surfplot_color <- xtabs(N ~ vel_abs_breaks_min + GD_breaks_min, 
                        data = toplot2 %>% filter(N>0))
surfplot_color[surfplot_color==0] <- NA

plot_ly(x = ~rownames(surfplot_color), 
        y = ~colnames(surfplot_color),
        z = ~t(surfplot)) %>%
  add_surface(surfacecolor=t(surfplot_color)) %>%
  layout(
    scene = list(
      xaxis = list(title = 'Velocity of isotherm shift'),
      yaxis = list(title = 'Genetic diversity'),
      zaxis = list(title = 'Velocity of range shift'),
      aspectmode='cube'))
...
legend <- cowplot::get_legend(
  gg3 +
    scale_color_gradientn(colors = rampPallFunGD(x=1),
                          guide = guide_colourbar(title.position = "right",
                                                  barwidth = 1, barheight = 15),
                          limits=c(0, 7),
                          breaks=c(seq(0,7,by=1))))
...
legend <- cowplot::get_legend(
  gg3 +
    scale_color_gradientn(colors = rampPallFunVel(x=1),
                          guide = guide_colourbar(title.position = "right",
                                                  barwidth = 1, barheight = 15),
                          limits=c(0, 7),
                          breaks=c(seq(0,7,by=1))))
