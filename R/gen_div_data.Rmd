...
# possible combinations Type x Param x ECO x Hemisphere
pos_mod <- expand.grid(Type = unique(biov1$Type),
                       Param = unique(biov1$Param),
                       ECO = unique(biov1$ECO),
                       Hemisphere = unique(biov1$Hemisphere))
pos_mod <- pos_mod %>%
    filter(!(ECO == "M" & Type == "ELE"))
N_data <- sapply(1:nrow(pos_mod), function(i){
    biov1 %>%
        filter(Type==pos_mod$Type[i],
               Param==pos_mod$Param[i],
               ECO==pos_mod$ECO[i],
               Hemisphere==pos_mod$Hemisphere[i]) %>%
        nrow
})
pos_mod <- pos_mod[-which(N_data == 0),]

size_data <- sapply(1:nrow(pos_mod), function(i){
    data <- biov1 %>%
        filter(Type==pos_mod$Type[i],
               Param==pos_mod$Param[i],
               ECO==pos_mod$ECO[i],
    ...
    Groups <- as.data.frame(table(data$Group))
    names(Groups) <- c("Group", "Freq")
    Groups=Groups[which(Groups$Freq>10), ] 
    
    data <- data %>% filter(Group %in% Groups$Group)
    
    nrow(data)
})
pos_mod <- pos_mod[-which(size_data == 0),]
...
for(i in 1:nrow(pos_mod)){ 
    ...
    # here I use a criteria of number of observation by Groups in order to have sufficient replicate among Groups (I set it to 10 but you can use another threshold)
    Groups <- as.data.frame(table(data$Group))
    names(Groups) <- c("Group", "Freq")
    Groups=Groups[which(Groups$Freq>10), ] 
    data <- data %>% filter(Group %in% Groups$Group)
    ...
    data_out[[i]] <- data
    #considered as random effect as it can drive bias in model that we want to control but it's not a truly methodological factor
    test_start <- ifelse(length(unique(data$StartF))>1,'y','n')
    test_UP <- ifelse(length(unique(data$Uncertainty_Parameter))>1,'y','n')
    
    x=list(if(test_start=='y'){"(1|StartF)"},
           if(test_UP=='y'){"(1|Uncertainty_Parameter)"})
    
    x=unlist(x)
    f=list("SHIFT ~ ",
           if(length(unique(data$Data))>1){"Data +"},
           if(length(unique(data$Sampling))>1){"Sampling +"},
           if(length(unique(data$Grain_size))>1){"Grain_size +"},
           if(length(unique(data$Uncertainty_Distribution))>1){"Uncertainty_Distribution +"},
           if(length(unique(data$ID.area))>1){"ID.area +"},
           if(length(unique(data$N))>1){"N +"},
           "(1|Family/Genus)")
    f=paste(unlist(f),collapse = "")
    ...
    if(nrow(tx1)==0){ #in case of no model selected considering our criteria (convergence and non signularity), a new random model structure is tested: (1|Family/Genus) + (1|Source)
        ...
        while(iS==T & z<=nrow(tx1)){
            ...
            mod1=lmer(as.formula(f1),
                      data,REML=TRUE,na.action="na.fail",
    ...
    if(isSingular(mod1)==T){ #in case of the selected model (with REML=T) is singular, a new random model structure selection is conducted substituing (1|Family/Genus) by (1|Genus)
        ...
        tx1=tx1[order(tx1$aicc,decreasing=F),] 
        if(nrow(tx1)==0){
            f1=paste(f,"+(1|ID)",sep="") #in case of no model selected considering our criteria (convergence and non signularity), a new random model structure is tested: (1|1/Genus) + (1|ID)
            mod1=lmer(as.formula(f1),data,REML=TRUE,na.action="na.fail",control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
        }else{
            z=1
            iS=T
    ...
    if(isSingular(mod1)==T){ #in case of the selected model (with REML=T) is singular, a new random model structure selection is conducted without any phylogenetic effect
        ...
        if(nrow(tx1)>0){
            ...
            while(iS==T & z<=nrow(tx1)){
                f1=paste(f,"+",tx1$test[z],sep="") #best model selection based on AICc
                mod1=lmer(as.formula(f1),data,REML=TRUE,na.action="na.fail",
        ...
        if(isSingular(mod1)==T){#in case of no model selected considering our criteria (convergence and non signularity), a more simple random model structure is tested: (1|Family/Genus)
            ...
            if(isSingular(mod1)==T){ #in case of singularity, a more simple random model structure is tested: (1|Genus)
                ...
                if(isSingular(mod1)==T){ #in case of singularity, a more simple random model structure is tested: (1|Genus)
                    ...
                    mod1=lmer(as.formula(f1),data,REML=TRUE,na.action="na.fail",
                              control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
                    if(isSingular(mod1)==T){ #in case of singularity, a more simple random model structure is tested: (1|ID)
                        f=list("SHIFT ~ ",
                               if(length(unique(data$Data))>1){"Data +"},
                               if(length(unique(data$Sampling))>1){"Sampling +"},
                               if(length(unique(data$Grain_size))>1){"Grain_size +"},
...
# Get statistics of the selected model
m_metr <- lapply(m_out, function(x){
    tmp = data.frame(model = as.character(x@call[2]), BIC = BIC(x),AIC=AIC(x),AICc=AICc(x))
    r2s = MuMIn::r.squaredGLMM(x)
    cbind(tmp,r2s)
}
)
m_metr <- do.call(rbind,m_metr)
m_metr <- cbind(pos_mod,m_metr)
m_metr

# save model statistics
write.csv(m_metr, "Data/corrected_shifts_models_statistics.csv", row.names = FALSE)

# Collect the residuals and predicted values
...
SHIFT_cor <- lapply(m_out, function(i){ summary(i)$residuals })
biov1 <- lapply(1:length(data_out), function(i){
    data.frame(data_out[[i]],SHIFT_cor=SHIFT_cor[[i]])
})
biov1 <- rbindlist(biov1)

# corrected shifts
write.csv(biov1, "Data/biov1_corrected_shifts.csv", row.names = FALSE)
...
biov1$lag2[position] <- biov1$velocity[position]
...
biov1$lagC2[position] <- biov1$velocity[position]
...
# Save data
```{r}
write.csv(gen_data_v1_dist2,"Data/gen_data_final.csv",row.names = FALSE)
```
